package com.hotsharp.search.util;
...
//            List<Object> vidList = videoMapper.selectObjs(queryWrapper);
            List<Video> videoList = videoClient.selectVideos(
                        new Video().setStatus(i).setDeleteDate(null)
                ).getData();
            List<Object> vidList = videoList.stream().map(Video::getVid).collect(Collectors.toList());;
            try {
...
//            List<ChatDetailed> list = chatDetailedMapper.selectList(queryWrapper);
            List<ChatDetailed> list = messageClient.selectChatDetailed(new ChatDetailed()).getData();
            // 按用户将对应的消息分类整理
        /*
        chatSet中的数据对象示意：
        [
