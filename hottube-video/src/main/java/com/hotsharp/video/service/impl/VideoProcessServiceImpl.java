...
//            url = uploadM3U8ToMinio(uploadId);
            url = uploadVideo(videoPath, uploadId);
        } catch (Exception e) {
...
//        deleteFile(new File(dir2));
    ...
    private String uploadVideo(String videoPath, String uploadId) {
        File file = new File(videoPath);
        if (!file.exists()) return "";
        minioUtil.upload(file, uploadId);
        return minioProperty.getBaseUrl() + FileConstant.MINIO_VIDEO_PREFIX + uploadId + "/" + uploadId + ".mp4";
    }
    ...
    private String uploadM3U8ToMinio(String uploadId) throws InterruptedException {
        ...
        replaceUrl(fullPath, minioProperty.getBaseUrl()+"videos/"+uploadId+"/");
